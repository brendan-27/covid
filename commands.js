...
bot.on("message", async msg=>  {
    ...
    if (cmd === `${prefix}global`) {
        ...
        Embed = new discord.MessageEmbed()
            ...
            .addField("☠️ Total Deaths:", numberWithCommas(deaths) + "(+" + numberWithCommas(newDeaths) + " today)" + "(" + Math.round(deaths / totalCases * 100)/10 + "% of total cases)", true)
            .addField("💉 Total Recovered: ", numberWithCommas(recovered) + "(" + Math.round(recovered / totalCases * 100)/10 + "% of total cases)", true)
            .addField("🗺️ Total Countries:", numberWithCommas(totalCountries), true)
            .addField("☠️ New Deaths Today: ", numberWithCommas(newDeaths) + "(" + Math.round(newDeaths / deaths * 100)/10 + "% of total deaths)", true)
            .addField("✉️ New Cases Today: ", numberWithCommas(newCases) + "(" + Math.round(newCases / totalCases * 100)/10 + "% of total cases)", true)
    ...
    if (cmd == `${prefix}countryinfo` || cmd == `${prefix}country`) {
        ...
        Embed = new discord.MessageEmbed()
            ...
            .addField("💀 Confirmed Deaths:", numberWithCommas(confrimedDeaths) + "(" + Math.round(confrimedDeaths / confirmedCases * 100)/10 + "% of total cases)", true)
            .addField("💉 Confirmed Recoveries:", numberWithCommas(recoveries) + "(" + Math.round(recoveries / confirmedCases * 100)/10 + "% of total cases)", true)
            .addField("💹 Active Cases:", numberWithCommas(activeCountry) + "(" + Math.round(activeCountry / confirmedCases * 100)/10 + "% of total cases)", true)
            .addField("💀 Critical:", numberWithCommas(criticalCountry) + "(" + Math.round(criticalCountry / continentCases * 100)/10 + "% of total cases)", true)
    ...
    if (cmd == `${prefix}statesinfo` || cmd == `${prefix}stateinfo` || cmd == `${prefix}state`) {
        ...
        Embed = new discord.MessageEmbed()
            ...
            .addField("☠️Confirmed Deaths:", numberWithCommas(deahtsState) + "(" + Math.round(deathsState / casesState * 100)/10 + "% of total cases)", true)
            .addField("🧪Tests:", numberWithCommas(tests), true)
            .addField("💹Active:", numberWithCommas(activeCases) + "(" + Math.round(activeCases / casesState * 100)/10 + "% of total cases)", true)
    ...
    if(cmd == `${prefix}continentsinfo` || cmd == `${prefix}continentinfo` || cmd == `${prefix}continent`) {
        ...
        Embed = new discord.MessageEmbed()
            ...
            .addField("💹 Active:", numberWithCommas(continentActive) + "(" + Math.round(continentActive / continentCases * 100/10) + "% of total cases)", true)
            .addField("📊 New Cases:", numberWithCommas(continentNewCases) + "(" + Math.round(continentNewCases / continentCases * 100)/10 + "% of total cases)", true)
            .addField("☠️ New Deaths:", numberWithCommas(continentTodayDeaths) + "(" + Math.round(continentTodayDeaths / continentDeaths * 100)/10 + "% of total deaths)", true)
            .addField("🏥 Recovered:", numberWithCommas(continentRecovered) + "(" + Math.round(continentRecovered / continentCases * 100)/10 + "% of total cases)", true)
